...
begin
  ...
  unless storage_domain_id.nil?
    ...
    body = builder.to_xml
    $evm.log(:info, "Before RHV call, body: #{body.inspect}")
    ...
    $evm.log(:info, "After RHV call, response: #{response.inspect}")
    $evm.log(:info, "After RHV call, response.body: #{response.body.inspect}")
    #
    # Parse the response body XML
    #
    doc = Nokogiri::XML.parse(response.body)
    $evm.log(:info, "After RHV call, doc: #{doc.inspect}")
    #
